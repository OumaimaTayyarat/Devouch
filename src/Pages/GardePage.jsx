...
const ThreeScene = () => {
// Ref for the plane (floor)
  let showGuides = false;

  // 👆 change to true to see the physics objects
  
  let extraBounce = 0.01;
  
  // 👆 change to 1 for bouncey fun! Change to 2 for everything to bounce out of existence!
  
  let powerMultiplier = 0.6   ;
  const [soundEnabled, setSoundEnabled] = useState(true); // L'état pour activer/désactiver le son

  const soundEnabledRef = useRef(soundEnabled); // Référence pour maintenir l'état à jour dans les callbacks
  ...
  const playSound = (name) => {
    if (!soundEnabledRef.current) return; // Ne joue pas le son si soundEnabled est false
  ...
  const toggleSound = () => {
    setSoundEnabled((prev) => {
      console.log("avant", prev); // Afficher l'état avant
      return !prev; // Inverser l'état
    });
  };
  ...
  useEffect(() => {
    ...
    class Stage 
    {
      constructor(scale, stageSize, onReady, showStats)
      {
        ...
        this.scene.add( new HemisphereLight(0x424874, 0x424874, 0.4 ));
        ...
        this.cannonLight = new PointLight( 0x424874, 0, 100, 2);
        ...
        this.plane = new Mesh(
          ...
          new MeshPhongMaterial  ( { color: 0x424874 } )
            );
        ...
        for (let i = 0; i < 3; i++) {
          ...
          let geometry = new IcosahedronBufferGeometry(3, 10);
        ...
        treeLoader.load("/web_page.glb", object => {
    
          object.scene.position.set(-1, -1, 10);
        ...
        // fireplaceLoader.load("/box-electronic_-_20mb.glb", object => {
    
        //   object.scene.position.set(0, -27, -14.5);
        //   object.scene.scale.set(6, 7, 6);
        ...
        tableLoader.load("/compressor_room1.glb", object => {
    
          object.scene.position.set(0, -3.75, 0);
          object.scene.rotation.y = -Math.PI * 0.5 -Math.PI * 0.5


          object.scene.scale.set(18,14,18);
        ...
        var tableLoader = new GLTFLoader(manager);
        tableLoader.load("/web_page.glb", object => {
        ...
        snowmanLoader.load("/web_page.glb", object => {
    
          //object.scene.position.set(30, -30, 30);
          object.scene.scale.set(0.2, 0.2, 0.2); // Réduit la taille à 10%
          // object.scene.rotation.y = Math.PI * 1.25;
        ...
        //    pneu
        ///=================
        let pneuGroup = new Group();
        pneuGroup.position.y = 1000;
        this.scene.add( pneuGroup );
        
        var starLoader = new GLTFLoader(manager);
        starLoader.load("/web_page.glb", object => {
    
          // object.scene.position.set(-1, 0, 0);
          object.scene.scale.set(0.05, 0.05,0.05); // Réduit la taille à 10%
          object.scene.rotation.y = Math.PI * .5;
          pneuGroup.add( object.scene );
        ...
        starLoader.load("/web_page.glb", object => {
          ...
          object.scene.position.set(-1, 0, 0);
          object.scene.scale.set(0.051, 0.051, 0.051);
          object.scene.rotation.y = Math.PI * .5;
    ...
    class Physics
    {
      ...
      createBody(mass, position, rotation, material = PHYSICS_MATERIAL.lowBounce)
      {
    ...
    function introCannon()
    {
      ...
      cannonTL.to(steveEl, {y: '-=20', duration: 0.1, onComplete: () => {
        steveEl.setAttribute('src', '/happy.svg');
        textEl.textContent = "Devouch propose des solutions digitales innovantes et sur mesure pour transformer vos idées en projets concrets, en répondant à tous vos besoins en développement web, applications mobiles, et plateformes digitales.";
    ...
    function introStart()
    {
      ...
      cannonTL.to('#bubble', {autoAlpha: 0, y:'-=30', scale: 0.5, duration: 0.3, ease: 'power2.in'}, 0);
      cannonTL.to(steveEl, {y: '-=20', duration: 0.1, onComplete: () => {
        steveEl.setAttribute('src', '/steve.svg');
        textEl.textContent = "Cliquer ou Tapez sur votre clavier pour découvrir une animation 3D captivante, reflet de notre esprit créatif et de nos compétences en développement digital  !";
    ...
    function endMessage()
    {
      ...
      // steveEl.setAttribute('src', 'https://assets.codepen.io/557388/snowman.svg');
      textEl.innerHTML = ` Nous sommes là pour résoudre ces défis et redonner à vos ateliers la couleur et l’énergie qu’ils méritent.`
    
    
    // Be sure to send a screenshot to <a href="https://twitter.com/steeevg/" target="_blank">@steeevg</a>, he’d love to see it!`;
      textHighlightEl.textContent = "Bienvenue!";
    ...
    function fire(coords)
    {
      ...
      if (count % 5 === 0 && count <= 95)item = createSnowman();
      if (count % 3 === 0 && count <= 99)item = createStar()
      else   if (count % 2 === 0 && count <= 98)item = createTree(); //randomItems[Math.floor(Math.random() * randomItems.length)]();
      else item = addBall();
    ...
    {		
      ...
      let width = 1 + Math.random() ;
      let height = 1 + Math.random() ;
      let depth = 1 + Math.random() ;
      
      let isBall = Math.random() > 0.2;
    ...
    function init()
    {
      ...
      {
        let body = physics.createBody(14, {x: 0, y: -13, z: 2}, {y: -Math.PI * 1.013, x: Math.PI * 0.01}, PHYSICS_MATERIAL.highBounce);
      ...
      function createTable() {
        // Donnons à la table une masse de 14 (plutôt que 0) pour qu'elle soit stable mais fixe
        let body = physics.createBody(0, { x: 0, y: -13, z: 2 }, { y: -Math.PI * 1.013, x: Math.PI * 0.01 }, PHYSICS_MATERIAL.highBounce);
    
        // Liste des formes pour la table et ses pieds
        let shapes = [
        ...
        // Positionner le modèle de la table à la hauteur correcte
        stage.models.table.position.y = 0;
    
        let tableGroup = new Group();
        tableGroup.add(stage.models.table);  // Ajouter le modèle de la table à la scène
        stage.scene.add(tableGroup);
    
        // Ajouter chaque forme de la table au corps physique
        shapes.forEach(box => {
            let shape = physics.createBoxShape(box.width, box.height, box.depth);
    
            if (showGuides) {
                let b = stage.createBox(box.width, box.height, box.depth, 0xff0000);  // Optionnel : afficher des guides
                b.position.set(box.x, box.y, box.z);
                b.rotation.set(0, 0, box.rotation);
                tableGroup.add(b);
            }
    
            // Ajouter chaque forme au corps physique avec sa position et sa rotation
            let quat = new CANNON.Quaternion();
            quat.setFromEuler(0, box.rotation, 0);  // Rotation autour de l'axe Y
            body.addShape(shape, new CANNON.Vec3(box.x, box.y, box.z), quat);
        });
    
        // Appliquer une masse pour que la table soit stable mais fixe
        body.mass = 0;  // Masse non nulle pour la stabilité
    
        // Fixer la table : annuler la vélocité et la vélocité angulaire pour empêcher tout mouvement
        body.velocity.set(0, 0, 0);  // Pas de mouvement linéaire
        body.angularVelocity.set(0, 0, 0);  // Pas de rotation
    
        // Position fixe de la table
        body.position.set(0, -25, 0);  // Position fixe
    
        // Créer l'objet physique avec le groupe d'objets et le corps physique
        let physicsItem = {
            mesh: tableGroup,
            physics: body,
        };
    
        // Ajouter l'élément physique à la liste des objets physiques
        physicsItems.push(physicsItem);
    }
    
      function createTree() {
        // Création du corps physique (si nécessaire)
        let body = physics.createBody(10, {x: 30, y: -10, z: 30}, {y: 0, x: Math.PI * .5}, PHYSICS_MATERIAL.lowBounce);
        physics.setAngle(body, -Math.PI * 0.5, 'x');
      
        // Positionnement du modèle de snowman existant
        stage.models.pneu.position.y = 0;
      
        // Ajout du snowman existant à un groupe
        let pneuGroup = new Group();
        pneuGroup.add(stage.models.pneu);
        stage.scene.add(pneuGroup);
      
        // Création de l'objet physique uniquement pour le groupe existant
        var physicsItem = {
          mesh: pneuGroup,
          physics: body,
        };
      
        // Ajout à la liste des éléments physiques
        physicsItems.push(physicsItem);
      ...
      function createStar() {
        ...
        let body = physics.createBody(1, {x: 30, y: -10, z: 30}, {y: 0, x: Math.PI * .5}, PHYSICS_MATERIAL.lowBounce);
        physics.setAngle(body, -Math.PI * 0.5, 'x');
        stage.models.star.position.y = 0;

        // Créer un groupe pour l'étoile
        let starGroup = new Group();
        starGroup.add(stage.models.star); // Ajouter uniquement l'étoile initiale
        stage.scene.add(starGroup); // Ajouter le groupe à la scène
    
        // Ajouter l'objet physique correspondant
    
        var physicsItem = { 
            mesh: starGroup, // Associer le groupe `starGroup` au système physique
            physics: body,
        };
    
        // Ajouter l'objet physique à la liste
        physicsItems.push(physicsItem);
    
        return physicsItem; // Retourner l'objet physique associé
    }
  ...
  return (
    ...
    <div id="ui">
        <div class="positioner">
            ...
            <div class="speech-bubble">
                <div id="bubble">
                <span id="text">Votre site web manque-t-il d'impact, vos projets numériques peinent-ils à voir le jour, et avez-vous du mal à trouver des développeurs innovants et créatifs pour donner vie à vos idées ?</span>
                <span id="text-highlight"></span>
                    <div id="next">Next</div>
                </div>
            </div>
    
                <img id="steve" src="/sad.svg" alt="Steve"/>
    ...
    <div className="logo-container2">
      <img
        ...
        style={{
          cursor: "pointer", // Rendre l'icône cliquable
          zIndex: 9999,
          pointerEvents: "auto", 
          width: "60px",/* Ajuster la taille du logo */
          height: "60px",
          // Permet à l'élément de recevoir les clics
